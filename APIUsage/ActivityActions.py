...
def Get_Liked_Activities(perPage, totalPages, include_message_activity):
    ...
    following_users = (
        Get_Following()
    )  # Assuming this function returns a list of user IDs
    ...
    for page in range(1, totalPages + 1):
        print(f"\nChecking page {page}...")
        query, variables = QM.Queries.User_Activity_Feed_Query(
            viewer_ID, page, perPage, include_message_activity, start_time, end_time
        )
        response = API_Request(query, variables)
        ...
        for activity in activities:
            ...
            if "likes" in activity:
                for user in activity["likes"]:
                    ...
                    if (
                        user_id not in following_users
                        and user_id not in unfollowed_ids
                        and follow_unfollowed_users
                        and user_id != viewer_ID
                        and user_id not in followed_users
                    ):
    ...
    for user_id in list(
        followed_users
    ):  # Use list() to create a copy of the list for iteration
        if (
            user_likes_count.get(user_id, 0) >= likes_threshold
        ):  # Only follow users who have liked more than the threshold number of activities
            if Follow_User(user_id):
                following_users.append(
                    user_id
                )  # Add the user ID to the following_users list
        else:
    ...
    for user_id, count in sorted_user_likes:  # Iterate over the sorted items
        if (
            count >= likes_threshold
        ):  # Only display users who have liked at least likes_threshold activities
